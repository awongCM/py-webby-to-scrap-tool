...
class JobSearchSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for job in job_articles:
            ...
            item = {}
            item['job_title'] = job.css('article::attr(aria-label)').extract_first()
            item['company'] = job.css('a[data-automation="jobCompany"]::text').extract_first()
            item['location'] = job.css('a[data-automation="jobLocation"]::text').extract_first()
            item['area'] = job.css('a[data-automation="jobArea"]::text').extract_first()
        ...
        next_page_url = response.css('a[data-automation="page-next"]::attr(href)').extract_first()
